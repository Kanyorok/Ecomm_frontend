...
import { chooseOptions as chooseOptionsAction } from "../../../redux/kamandoraSlice";
...
const ProductBanner = ({ itemsPerPageFromBanner }) => {
  ...
  const chooseOptions = useSelector(
    (state) => state.kamandoraReducer.chooseOptions
  );
  ...
  const handleChooseOption = (option) => {
    dispatch(chooseOptionsAction(option));
  };
        ...
        {/* <div className="flex items-center gap-2 text-base text-[#767676] relative">
          ...
          <select
            onChange={(e) => {
              ...
              const selectedOption = showOptions.find(
          ...
          >
            {showOptions.map((item) => (
              <option key={item._id} value={item.section}>
                {item.title}
              </option>
            ))}
          </select>
