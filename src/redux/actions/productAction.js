...
export const getProducts = (keyword = '',currentPage = 1, price, category, rating=0) => async (dispatch) => {
    try{
        ...
        let link = `${apiUrl}/api/v1/products?page=${currentPage}`

        if(category){
            link = `${apiUrl}/api/v1/products?page=${currentPage}&category=${category}`
        }
...
export const updateProduct = (id, productData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`${apiUrl}/api/v1/admin/product/${id}`, productData, config)
...
export const deleteReview = (id, productId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.delete(`${apiUrl}/api/v1/reviews?id=${id}&productId=${productId}`)
